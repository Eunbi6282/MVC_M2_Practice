...
<body>
	...
	<!--  Form 태그 내에서 input type file이 존재하면
	 method는 반드시 post여야 한다. enctype의 "multipart/form-date"가 반드시 있어야 함 <-- 라이브러리를 통해서 업로드 지원
	 
	 <주의> : 서버에서 request.getParameter("name") : request객체를 사용하면 안된다.
	 		: 라이브러리에서 지원해주는 객체의 메서드로 폼에 있는 변수값을 받아야 한다.
	 		참고 : 라이브러리마다 메서드 이름이 다를 수 있다. 	 
	 -->
	<form name = "writeFrm" method = "post" enctype = "multipart/form-data" action = "../mvc_board/write.do"
